...
const Teach = () => {
    ...
    const checkNameUnique = async (name) => {
        const { data } = await instance.get(
            `admin/majors/check-unique/${name}`
        );
        if (data.is_unique) {
            return true;
        } else {
    ...
    const onHandleDelete = async (id) => {
        ...
        try {
            ...
            message.success(
                <span>
                    Xóa mềm thành công,
                    <button onClick={() => undoMajor(id)} className="underline">
                        Hoàn tác
                    </button>
                </span>,
                5
    ...
    const currentMajors = (
        filterMajors.length > 0 ? filterMajors : majors
    ).slice(indexOfFirstMajor, indexOfLastMajor);
        ...
        <>
            ...
            <div className="">
                <div className="row row-cols-2 g-3">
                    ...
                    <Modal
                        title={
                            update ? "Cập nhật ngành học" : "Thêm mới ngành học"
                        }
                        open={isModalVisible}
                                                    ...
                                                    if (!update) {
                                                        ...
                                                        if (!isUnique) {
                                                            return Promise.reject(
                                                                new Error(
                                                                    "Tên ngành đã trùng, vui lòng đặt tên khác."
                                                                )
                                                            );
                                                        }
                                                    }
