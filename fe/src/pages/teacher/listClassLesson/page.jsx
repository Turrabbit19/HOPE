...
export default function ScheduleTable() {
    ...
    const handleStudentStatsClick = async (scheduleId) => {
        ...
        try {
            ...
            if (
                result &&
                result.ListStudents &&
                Array.isArray(result.ListStudents)
            ) {
                                            ...
                                            {student.absent_details.map(
                                                (lesson, lessonIndex) => {
                                                    ...
                                                    const hasOccurred =
                                                        lessonDate &&
                                                        lessonDate <=
                                                            currentDate; // Check if the lesson date has passed
                                                    const status = lesson
                                                        ? lesson.status
                                                        : "Chưa rõ"; // Default to "Chưa rõ" if status is missing
    ...
    if (error)
        return (
            <div className="text-center py-6 text-red-600 text-2xl">
                Lỗi: {error}
            </div>
        );
    if (!schedules.length)
                ...
                <div className="overflow-x-auto">
                    <table className="w-full table-auto">
                        <thead className="bg-gray-100">
                            <tr>
                                <th className="text-2xl px-6 py-4 text-left font-semibold text-gray-700 uppercase tracking-wider">
                                    Lớp học
                                </th>
                                <th className="text-2xl px-6 py-4 text-left font-semibold text-gray-700 uppercase tracking-wider">
                                    Môn học
