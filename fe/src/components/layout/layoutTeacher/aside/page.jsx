...
const AsideTeacher = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await instance.get(`teacher/noti-change-schedule`);
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await instance.get(`teacher/get-sub-schedules`);
  ...
  return (
    <aside className="bg-gradient-to-br from-teal-50 to-blue-50 shadow-lg min-h-screen p-6 border-r border-black transition-all duration-300 ease-in-out ">
      ...
      <div className="flex items-center mb-8">
        <img src="/public/assets/img/download (10).jpg" alt="Logo" width={50} />
      </div>

      {/* Trang chủ */}
      <div className="mb-6">
        <h2 className="mb-3 font-semibold text-gray-800 text-3xl">Trang chủ</h2>
      ...
      <div className="mb-6">
        ...
        <nav className="space-y-2">
          <NavLink
            ...
            aria-current={
              activeLink === "/listclasslesson" ? "page" : undefined
            }
          >
            <span
              className={`p-2 rounded-md bg-white shadow-sm transition-all duration-200 ease-in-out mr-3 ${
            ...
            <span
              className={`font-medium ${
                activeLink === "/listclasslesson" ? "font-semibold" : ""
              }`}
            >
              Danh sách lớp học
            </span>
      ...
      {/* Lịch dạy */}
      <div className="mb-6">
        <h2 className="mb-3 font-semibold text-gray-800 text-3xl">Lịch dạy</h2>
        <nav className="space-y-2">
          <NavLink
            to="timetable"
            className={({ isActive }) =>
            ...
            aria-current={activeLink === "/timetable" ? "page" : undefined}
          >
            <span
              className={`p-2 rounded-md bg-white shadow-sm transition-all duration-200 ease-in-out mr-3 ${
            ...
            <span
              className={`font-medium ${
                activeLink === "/timetable" ? "font-semibold" : ""
              }`}
            >
              Lịch dạy
            </span>
          ...
          <div className="mt-4">
            ...
            >
              <Descriptions.Item label="Môn học">
                {notifications?.subject_name || "Không có thông tin"}
              </Descriptions.Item>
              <Descriptions.Item label="Phòng">
                {notifications?.room_name || "Không có thông tin"}
              </Descriptions.Item>
              <Descriptions.Item label="Ca học">
                {notifications?.shift_name || "Không có thông tin"}
              </Descriptions.Item>
              <Descriptions.Item label="Ngày">
                {notifications?.date
                  ? dayjs(notifications.date).format("DD/MM/YYYY")
                  : "Không có thông tin"}
              </Descriptions.Item>
              <Descriptions.Item label="Người yêu cầu">
                {notifications?.requester_name || "Không có thông tin"}
              </Descriptions.Item>
            </Descriptions>
          </div>
        ) : (
      ...
      {subSchedules && Object.keys(subSchedules).length > 0 ? (
        <nav className="space-y-2">
          <NavLink
            to="sub-schedules"
            className={({ isActive }) =>
          ...
          >
            ...
            <span
              className={`font-medium ${
                activeLink === "/sub-schedules" ? "font-semibold" : ""
              }`}
            >
              Lịch dạy bù
            </span>
