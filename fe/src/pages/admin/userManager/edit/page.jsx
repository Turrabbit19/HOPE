...
const UserEdit = () => {
  ...
  function initValue(res) {
    ...
    if (state.type === "students") {
      form.setFieldValue(
        "course",
        listCourse.find((e) => (e.name = res.data.data["course_name"])).id
      );
      form.setFieldValue(
        "major",
        mainMajor.find((e) => (e.name = res.data.data["major_name"])).id
      );
    } else if (state.type === "teachers") {
      form.setFieldValue(
        "major",
  ...
  const onFinish = async (values) => {
    try {
      ...
      if (avatar) {
        ...
        data = {
          ...data,
          avatar: uploadResults ? uploadResults[0]?.data?.fullPath : null,
        };
      }
