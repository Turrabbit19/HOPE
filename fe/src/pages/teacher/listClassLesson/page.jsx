...
export default function ScheduleTable() {
  ...
  const StudentStatsPopup = ({ data, onClose }) => {
    ...
    return (
      ...
      >
        <div
          className="bg-white shadow-md rounded-lg p-8 max-w-[95%] w-auto max-h-[90vh] overflow-auto"
          onClick={(e) => e.stopPropagation()}
        >
            ...
            <thead className="bg-gray-200 text-gray-700">
              <tr>
                ...
                {data[0].absent_details.map((_, index) => (
                  <th key={index} className="border border-gray-400 px-4 py-3">
                    Tiết {index + 1}
                  </th>
                ))}
              </tr>
  ...
  if (loading)
    return (
      <div className="text-center py-4 animate-pulse text-gray-600">
        Đang tải...
      </div>
    );
  if (error)
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                <th className="text-xl px-4 py-3 text-left font-medium text-gray-500 uppercase tracking-wider">
                  Lớp học
                </th>
