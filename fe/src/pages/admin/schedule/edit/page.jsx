...
const EditSchedule = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const [{ data }, majors, courses, classrooms, rooms, teachers] = await Promise.all([
          instance.get(`admin/schedules/${id}`),
        ...
        form.setFieldsValue({
            startDate: moment(data.data.start_date),
            endDate: moment(data.data.end_date),
            session: data.data.shift_name,
            repeatDays: data.data.days_of_week.map(item => {return `Thứ ${item.day}`}),
            learningMethod: data.data.room_name ? "Trực tiếp" : "Trực tuyến",
            teacher: data.data.teacher_name,
            course: data.data.course_name,
            semester: data.data.semester_name,
            subject: data.data.subject_name,
            major: data.data.major_name,
    ...
    if (selectedCourseId && selectedMajorId && selectedSemesterId) {
      try {
        ...
        const response = await instance.get(
          `admin/course/${selectedCourseId}/semester/${selectedSemesterId}/major/${selectedMajorId}/subjects`
        );
  ...
  const handleFinish = async (values) => {
    ...
    try {
      setLoading(true);
      await instance.put(`/admin/schedules/${id}`, convertToScheduleJson(values));
